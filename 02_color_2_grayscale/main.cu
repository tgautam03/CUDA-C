...
#include "color_2_gray_cpu.h"
#include "color_2_gray_gpu.h"
...
int main(int argc, char const *argv[])
{
    ...
    color_2_gray_cpu(P_in, P_out_cpu, N, M);
    
    printf("Printing color_2_gray_cpu: \n");
    for (int i = 0; i < N; i++)
    {
        for (int j = 0; j < M; j++)
        {
    ...
    color_2_gray_gpu(P_in, P_out_gpu, N, M);
    
    printf("Printing color_2_gray_gpu: \n");
    for (int i = 0; i < N; i++)
    {
        for (int j = 0; j < M; j++)
        {
